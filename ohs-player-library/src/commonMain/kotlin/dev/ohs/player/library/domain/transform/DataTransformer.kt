...
/**
 * Transforms a FHIR [Resource] into a Kotlin data class
[T] driven by a [ViewDefinition].
 *
 * Evaluates each FhirPath expression declared in the
ViewDefinition's columns against
 * the provided resource, then decodes the resulting
values into an instance of [T].
 *
 * Example:
 * ```kotlin
 ...
 *             ViewColumn(name = "patientId",  path =
"id"),
 *             ViewColumn(name = "familyName", path =
"name.family.first()")
 *         )))
 *     )
 * )
 ...
 * ```
 */
class DataTransformer(private val fhirPathEngine:
                      FhirPathEngine) {

    /**
     * Evaluates each FhirPath column declared in
    [viewDefinition] against [resource]
     * and decodes the result into an instance of [T].
     *
     * @param resource the FHIR resource to extract data
    from
     * @param viewDefinition declares the columns (name →
    FhirPath) to evaluate
     * @param contextVariablesMap optional named variables
    available during FhirPath evaluation
     * @return an instance of [T] with fields populated
    ...
    /**
     * Evaluates each FhirPath column in [viewDefinition]
    against [resource]
    ...
    companion object {
        /** Creates a [DataTransformer] configured for
        FHIR R4. */
        fun forR4(): DataTransformer =
            DataTransformer(FhirPathEngine.forR4())
    }
}