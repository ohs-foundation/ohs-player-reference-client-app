 ...
 *
 * Populated at app start (typically inside a `buildAppViewRegistry()` helper) and then
 * installed into the composition via [LocalViewRegistry] so scaffolds can resolve
...
class ViewRegistry {
    /**
     * Storage entry pairing the original [ComponentRenderer] with its config and the
     * pre-built [ConfiguredRenderer] wrapper. [configured] is built once at construction
     * so lookups are O(1) and the same wrapper instance is returned on repeated calls.
     *
     ...
     * [withConfig] was applied. Intended for identity-based tests and debugging.
     *
     * @throws NoSuchElementException if no component is registered for [key].
     */
    @Suppress("UNCHECKED_CAST")
 ...
 * type [T]. Useful for identity-based assertions in tests and runtime introspection;
